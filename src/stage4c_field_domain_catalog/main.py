...
from src.stage4_domain_typing.ground_truth_eval import (
    build_field_domain_catalog_only_from_ground_truth,
    evaluate_stage4_extraction,
    load_stage4_ground_truth,
)
from src.stage4c_field_domain_catalog.llm_field_domain import build_field_domain_catalog_openai
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Stage4c: field_domain_catalog (allowed values per field) via OpenAI or ground truth",
    )
    ...
    parser.add_argument("--ground-truth", type=str, default=None, metavar="PATH")
    parser.add_argument(
        "--ground-truth-as-output",
        action="store_true",
        help="Write field_domain_catalog.jsonl from --ground-truth only.",
    )
    add_rag_arguments(parser)
    args = parser.parse_args()

    run = StageRun.create("stage4c_field_domain_catalog")
    out_dir = artifact_path("stage4c_field_domain_catalog")
    ...
    if args.ground_truth_as_output:
        ...
        rows = build_field_domain_catalog_only_from_ground_truth(gt, run)
        write_jsonl(out_dir / "field_domain_catalog.jsonl", rows)
        write_json(
        ...
        print(f"[GT-as-output] field_domain_catalog rows={len(rows)}")
        return
    ...
    if args.ground_truth:
        ...
        report = evaluate_stage4_extraction(
            ...
            rows,
            {
                ...
                "field_domain_catalog": gt.get("field_domain_catalog") or [],
            },
        )
    ...
    write_json(out_dir / "run_manifest.json", run.to_dict())
    print(f"field_domain_catalog rows={len(rows)}")
